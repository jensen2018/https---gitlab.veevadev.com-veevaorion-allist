...
from orionbase.common.models import DailyReportedData, PageListConditions, User, Positions, PositionHistories, PageLists
...
class ReportClass(object):
    def detail(self, *args):
        ...
        object_name = 'daily_reported_data'
        ...
        raw_report_data = DailyReportedData.advance_objects.filter(pk__in=record_ids)
        now = datetime.today()
        ...
        day_condition = Q(report_date__gte=day_start.strftime('%Y-%m-%d')) & \
                         Q(report_date__lte=day_end.strftime('%Y-%m-%d'))
        end_date_of_month = monthrange(now.year, now.month)
        month_condition = Q(report_date__gte=now.strftime('%Y-%m') + '-01') & \
                          Q(report_date__lte=now.strftime('%Y-%m') + '-{0}'.format(end_date_of_month[1]))


        number_of_new_patients_on_week = raw_report_data.filter(
            day_condition # & Q(created_by_id=current_user.user_id)
        )
        data['week_patients_num'] = number_of_new_patients_on_week.aggregate(Sum('num_of_new_increased')).\
            get('num_of_new_increased__sum') if number_of_new_patients_on_week.exists() else 0
        number_of_new_patients_on_month = raw_report_data.filter(
            month_condition # & Q(created_by_id=current_user.user_id)
        )
        data['month_patients_num'] = number_of_new_patients_on_month.aggregate(Sum('num_of_new_increased'))\
            .get('num_of_new_increased__sum') if number_of_new_patients_on_month.exists() else 0
        return data
